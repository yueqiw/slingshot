...
setMethod(
  f = "get_curves",
  signature = signature(X = "matrix", cluster = "character", lineages = "list"),
  definition = function(X, cluster, lineages, shrink = TRUE) {
    thresh = 0.0001
    maxit = 100
    stretch = 2
    ...
    out <- list()
    out$curves <- lapply(pcurves,function(pc){pc$s})
    out$pseudotime <- sapply(pcurves,function(pc){
      ############ match or left join #############
    })
    return(pcurves)
  })
