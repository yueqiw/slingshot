    ...
    if(lineages){
      ...
      if(show.constraints){
        if(any(linC$start.given)){
          points(centers[clusters %in% linC$start.clus[linC$start.given],,
                  ...
                  for(jj in (ii+1):nclus){
                    if(forest[ii,jj]==1){
                      if(clusters[ii] %in% linC$start.clus | 
                        ...
                        seg.col <- 'green3'
                      }else if(clusters[ii] %in% linC$end.clus[linC$end.given] |
                               clusters[jj] %in% linC$end.clus[linC$end.given]){
                        seg.col <- 'red2'
                      }else{
                        seg.col <- 1
                      }
