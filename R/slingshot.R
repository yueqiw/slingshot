...
#' @param omega (optional) numeric, granularity parameter between 0 and 1.
#'   \code{omega} determines the distance between every real cluster and the
#'   artificial cluster, \code{OMEGA}. It is parameterized as a fraction of the
#'   largest distance between two real clusters (hence, any value greater than 1
#'   would result in a single tree being returned and would be equivalent to
#'   setting \code{omega = Inf}, which is the default behavior).
